...
client.on("message", async message => {
  if (message.author.id == "734350598740246589") {
...
client.on("ready", () => {
  setInterval(() => {
    let botdurum = client.channels.find(c => c.id === "734350598740246589");
...
client.on("message", async message => {
  ...
  if (
    ...
    if (i === "acik") {
      ...
      message.channel.send(embed).then(msg => msg.delete(5000000));
    }
  }
});
  ...
  db.fetch(`dm_${member.guild.id}`).then(i => {
    if (i == "acik") {
      const msj = new Discord.RichEmbed()
        .setColor("BLUE")
        .setDescription(
          `<@${member.user.id}> sunucuya hoşgeldin!\nKuralları Okumayı Unutma !\nKomutlarımı görmek için: !yardım\nEğer beni eklemek istersen: [[Tıkla!]](https://discord.com/api/oauth2/authorize?client_id=644234599601274912&permissions=8&redirect_uri=https%3A%2F%2Fcanim-bot.glitch.me%2Fcallback&scope=bot)`
        )
...
client.on("guildCreate", async guild => {
  ...
  client.channels.get(`734350598740246589`).send(embed);
});
    ...
    if (dogrulama !== null && gun - (Date.now() - dogrulama) > 0) {
          ...
          if(message.author.id === ayarlar.sahip){
           db.set(`sahiponay_${message.author.id}_${message.guild.id}`, Date.now())
            message.channel.send(":crown:  **Hizaya Geçin, Bu Benim Yapımcım** :crown: ").then(msg => msg.delete (15000))
            }
        }
